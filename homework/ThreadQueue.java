 ...
 * 开始网上抄的代码https://blog.csdn.net/qq_25343557/article/details/78010203 发现有点不对
 * 虽然消费者和生产者可以交换着取出、插入队列 但是get没返回值 改成while循环之后有返回了 感觉有点鸡肋 线程的Runable是不返回的
 * 取到了queue.get()也没啥用 不过可以后续做些什么操作 可能会有用
 */
            ...
            new Thread(() -> {
                while (true) {
                    ...
                    System.out.println(String.format("%s 从队列取走一个元素[%s]，队列长度：%s", Thread.currentThread().getName(),
                            String.valueOf(e), queue.getSize()));
                }
            }, "消费者" + i).start();
        }
